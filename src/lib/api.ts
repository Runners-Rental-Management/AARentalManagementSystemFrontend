import type { Property, User, UserRole } from "@/lib/types";
...
async function apiRequest<T>(
  ...
  if (!response) {
    ...
    throw new Error(
      `Cannot reach backend API. Tried: ${API_BASE_URLS.join(", ")}. Details: ${details}`
    );
  }
...
export async function apiListProperties(token: string, query?: string) {
  ...
  const result = await apiRequest<PropertyListResponse>(`/properties${suffix}`, { token });
  return {
    ...result,
...
export async function apiCreateProperty(token: string, payload: {
  title: string;
  address: string;
  subCity: string;
  woreda: string;
...
function mapBackendAgreement(raw: BackendAgreement): TenancyAgreement {
  ...
  return {
    ...
    tenantSignedAt: raw.tenantSignedAt ? toIsoDate(raw.tenantSignedAt) : undefined,
    landlordSignedAt: raw.landlordSignedAt ? toIsoDate(raw.landlordSignedAt) : undefined,
    verifiedAt: raw.verifiedAt ? toIsoDate(raw.verifiedAt) : undefined,
...
export async function apiListAgreements(token: string, query = "page=1&pageSize=100") {
  const result = await apiRequest<AgreementListResponse>(`/agreements?${query}`, {
    token,
  });
  return {
    ...result,
...
export async function apiGetAgreementById(token: string, id: string) {
  const result = await apiRequest<BackendAgreement>(`/agreements/${id}`, { token });
...
export async function apiListDisputes(token: string, query = "page=1&pageSize=100") {
  const result = await apiRequest<DisputeListResponse>(`/disputes?${query}`, { token });
  return {
    ...result,
