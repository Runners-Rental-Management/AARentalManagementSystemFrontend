...
export default function LandlordCounterSignPage() {
  const params = useParams();
  const router = useRouter();
  const { user } = useAuth();
  const { agreements: liveAgreements, landlordSign } = useRentalFlow();
  ...
  const property = agreement
    ? properties.find((p) => p.id === agreement.propertyId)
    : null;
  ...
  const handleSubmit = async () => {
    ...
    await withLoading(async () => {
      ...
      landlordSign(agreementId, png);
    }, locale === "am" ? "counter-sign እያስገቡ ነው…" : "Submitting your counter-signature…");
    setSubmitting(false);
    setSubmitted(true);
  };
          ...
          <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-start gap-3">
            ...
            <div className="text-sm text-amber-800">
              <p className="font-semibold mb-0.5">
                {locale === "am" ? "የተከራይ ፊርማ ተጠናቋል" : "Tenant signature complete"}
              </p>
              <p className="text-xs">
                {locale === "am"
                  ? "Counter-sign ከማድረግዎ በፊት የተከራይ ፊርማ እና ውሉን ይገምግሙ።"
                  : "Review the tenant's signature and contract terms before applying your counter-signature."}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
            <div className="bg-gradient-to-r from-primary-700 to-indigo-700 px-6 py-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-primary-200 text-xs">
                  Addis Ababa Rental Control System · Proclamation 1320/2016
                </p>
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 bg-slate-50/60 px-8 py-5 grid sm:grid-cols-2 gap-6 text-sm font-[Georgia,serif] text-slate-700">
              ...
              <div>
                ...
                <p className="mt-2">
                  ፊርማ፡{" "}
                  <span className="border-b border-emerald-400 inline-block w-36 text-emerald-600 font-semibold text-xs">
            ...
            <>
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
                <div className="flex items-center gap-2 text-xs text-emerald-600 font-semibold mb-5 bg-emerald-50 border border-emerald-200 px-3 py-2 rounded-lg w-fit">
                  ...
                  {locale === "am"
                    ? "ውሉን አንብበዋል — ፊርማ ክፍሉ ተከፈተ"
                    : "Contract read — signing section unlocked"}
                </div>
                ...
                <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 flex items-start gap-3 text-xs text-slate-600 leading-relaxed">
                  ...
                  <span>
                    {ct("eSignLegal")}
                    {locale !== "am" && <> (<strong>{agreement.landlordName}</strong>)</>}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                ...
                {(!agreed || !hasSig) && (
                  <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-2 text-xs text-amber-800">
                    ...
                    <span>{!hasSig && !agreed ? ct("lockBoth") : !hasSig ? ct("lockSig") : ct("lockCheck")}</span>
                  </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    {submitting ? (
                      ...
                      <>
                        <FileSignature className="w-4 h-4" />
                        {locale === "am" ? "Counter-Sign & Submit" : "Counter-Sign & Submit"}
                      </>
                    )}
                  </button>
                </div>
